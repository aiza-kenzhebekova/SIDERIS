...
def main():
    ...
    --------------------------------------------------------------------------------------------------------
    Format: python drop_out_test.py "directory" "instrument" "outputfilename"

    Output: .dat file of the photometry and FWHM (in arcsec) of sources which do not drop out

    Directory: File path of this package 
    ...
    linder_file = os.path.join(data_dir, "RXJ0534_CMD/BEX_evol_mags_-2_MH_0.00.dat")
    ...
    if instrument == "NIRCam":
        # read in photometry 
        mags_file = os.path.join(data_dir, "NIRCam/JADES_photometry.dat")
        mags_df = Table.read(mags_file, format = 'ascii')
        ...
        filt1_file = os.path.join(data_dir, "NIRCam/hlsp_jades_jwst_nircam_goods-s-deep_f200w_v2.0_drz.fits")
        filt2_file = os.path.join(data_dir, "NIRCam/hlsp_jades_jwst_nircam_goods-s-deep_f444w_v2.0_drz.fits")
        filt3_file = os.path.join(data_dir, "NIRCam/hlsp_jades_jwst_nircam_goods-s-deep_f356w_v2.0_drz.fits")
    ...
    if instrument == "MIRI":
        ...
        # get data in the three filters
        filt1_file = os.path.join(data_dir, "MIRI/hlsp_smiles_jwst_miri_goodss_f1500w_v1.0_drz.fits")
        filt2_file = os.path.join(data_dir, "MIRI/hlsp_smiles_jwst_miri_goodss_f2100w_v1.0_drz.fits")
        filt3_file = os.path.join(data_dir, "MIRI/hlsp_smiles_jwst_miri_goodss_f560w_v1.0_drz.fits")
    ...
    np.savetxt(outputfilename, save_photometry, header=column_titles,comments="", fmt="%s")
